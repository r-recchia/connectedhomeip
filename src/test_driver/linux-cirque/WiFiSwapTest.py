...
DEVICE_CONFIG = {
    ...
    'device1': {
        'type': 'CHIPEndDevice',
        'base_image': '@default',
        'capability': ['WiFi', 'TrafficControl', 'Mount'],
        'docker_network': 'Ipv6',
        'traffic_control': {'latencyMs': 100},
        "mount_pairs": [[CHIP_REPO, CHIP_REPO]],
        "description": [
...
class TestWiFiSwap(CHIPVirtualHome):
        ...
        for server in servers:
            self.execute_device_cmd(
                ...
                ("CHIPCirqueDaemon.py -- run gdb -return-child-result -q -ex \"set pagination off\" "
                 "-ex run -ex \"bt 25\" --args {} --discriminator {}").format(
...
if __name__ == "__main__":
    sys.exit(TestWiFiSwap(DEVICE_CONFIG).run_test())
