...
async def main():
    ...
    optParser.add_option(
        ...
        default="",
        type="str",
        help="WiFi password to connec to."
    )
    optParser.add_option(
        "-t",
        "--timeout",
    ...
    logger.info("Setting Wi-Fi credentials")
    FailIfNot(await test.SetWiFiNetwork(ssid=options.ssid, password=options.wifiPassword),
    ...
    logger.info("Setting Wi-Fi 2 credentials")
    FailIfNot(await test.SetWiFiNetwork(ssid=WIFI_NETWORK_2.ssid, password=WIFI_NETWORK_2.wifiPassword),
              "Failed to set Wi-Fi 2 credentials.")
